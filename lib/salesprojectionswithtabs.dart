...
List<String> data = ["Sales Projections"];
...
saveDataToPrefs() async {
  ...
  await prefs.setString('datab', dataJson);
}

loadDataFromPrefs() async {
  SharedPreferences prefs = await SharedPreferences.getInstance();
  String? dataJson = prefs.getString('datab');
  if (dataJson != null) {
...
class Futuretabs extends StatefulWidget {
  const Futuretabs({super.key});

  @override
  MyHomePageState createState() => MyHomePageState();
}

class MyHomePageState extends State<Futuretabs> {
  ...
  Future<void> _fetchData() async {
    final response = await http.get(Uri.parse('${ApiConstants.baseUrl}api.php?type=2'));
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => NoteDetailapi(_datab[0], "Edit CLient","","future"),
            ),
          );
        },
...
class CustomTabViewfuture extends StatefulWidget {
  const CustomTabViewfuture({
    super.key,
...
class CustomTabsState extends State<CustomTabViewfuture>
  ...
  @override
  void didUpdateWidget(CustomTabViewfuture oldWidget) {
                          ...
                          child: ReorderableListView(
                            onReorder: (oldIndex, newIndex) {
                              setState(() {
                                ...
                                final item = matchedElements[oldIndex];
                                matchedElements.removeAt(oldIndex);
                                matchedElements.insert(newIndex, item);

                                // Update the order in the _data list
                                final dataItem = _data[oldIndex];
                                _data.removeAt(oldIndex);
                                _data.insert(newIndex, dataItem);
                              });
                            },
                                    ...
                                    child: ListTile(
                                      ...
                                      onTap: () {
                                        ...
                                        Navigator.push(
                                          context,
                                          MaterialPageRoute(
                                            builder: (context) => NoteDetailapi(matchedElements[index], "Edit CLient","edit","future"),
                                          ),
                                        );
